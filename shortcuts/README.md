...
**Known limitation, not yet closed: does not cover `raspberry5-carolus-ros2/technical-ros2.tex`.** The ROS2 manual has the same "check without asking" governance as `overleaf/technical.tex` (`CLAUDE.md`'s "ROS2 manual" section) but no mechanical tool of its own — every value, listing, and command it cites has to be checked against the real source by hand. Found 2026-08-22, after three same-day editing passes on that manual with no automated stale-reference check available the whole time (everything was verified anyway, by direct comparison, but the safety net this script provides for the other manual is absent here). Extending this script (or writing a sibling) to cover both manuals is real, undone work — not attempted yet because the two manuals' self-sufficiency rules differ (this one may cite `research-log/`, the ROS2 one explicitly may not), so a shared implementation needs a parameter for that, not a copy-paste.
