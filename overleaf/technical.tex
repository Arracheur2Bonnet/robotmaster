...
\begin{itemize}
  ...
  USB). The Pi holds the single connection to the robot's SDK and runs the
  camera/beacon-tracking node.
  \item \textbf{Lab PC:} runs the operator GUI and, in the
  currently-validated configuration, the Carolus pose-estimation node. Not
  physically connected to the robot; communicates with the Pi over the local
  network. \textbf{Carolus is intended to move onto the Raspberry Pi} so the
  robot can operate without a network link to the PC ---
  see Section~\ref{sec:where-carolus-runs}.
...
This is a private repository; you need an SSH key registered against it
(ask the project supervisor for collaborator access if you do not already
have it) --- do not substitute a personal access token pasted into the
clone URL for this. If cloning over SSH is not set up, an HTTPS clone with
your own GitHub credentials works the same way:
...
camera stream, chassis drive, gimbal control, odometry, and IMU all go
through this one script and its one connection.
...
\texttt{/imu} & Raw IMU (accelerometer + gyroscope), 50\,Hz \\
...
Expect a steady rate on \texttt{/camera/color/image\_raw} (roughly
10--15\,Hz) and at least one non-empty message on \texttt{/odom} and
\texttt{/imu}. If the camera topic is silent, confirm the RNDIS link
...
\runson{the \textbf{lab PC} in the configuration documented here --- see Section~\ref{sec:where-carolus-runs} for why, and for the planned move onto the Raspberry Pi.}

Carolus (the ROS package name is \texttt{carolus\_astrobee}) is built and run
on the \textbf{lab PC} in the configuration this manual documents. It has no
SDK connection of its own --- it only consumes
\texttt{/camera/color/image\_raw} (published by \texttt{rm\_cam\_beacon.py}
over the network from the Pi, Chapter~\ref{ch:camera}) and publishes a pose.

\section{Where Carolus runs, and why it should move to the Pi}
\label{sec:where-carolus-runs}

\textbf{Running Carolus on the lab PC is the currently-validated
configuration, not the recommended end state.} It has a structural cost that
matters for autonomous operation, and the intended target is the Raspberry
Pi. Both are stated here rather than presenting the current setup as
settled.

\subsection{The cost of running it on the PC}

Two consequences follow directly from the split:

\begin{itemize}
  \item \textbf{The robot cannot operate away from the PC.} Every pose
  estimate depends on a live network link to the lab PC, so the robot must
  stay within network reach at all times --- incompatible with genuinely
  autonomous operation.
  \item \textbf{It is very likely the throughput bottleneck.} Carolus
  currently publishes poses at roughly 2.5\,Hz on this setup.
  \texttt{/camera/color/image\_raw} is published as \emph{uncompressed}
  \texttt{sensor\_msgs/Image} at $1280\times720$ --- about 2.76\,MB per frame
  in BGR8 --- and every frame must cross the Pi$\to$PC link before Carolus
  can process it. Published uVGS-2 benchmarks on comparable CPU hardware
  report throughput roughly $40\times$ higher than what is observed here,
  which places the P4P/Ceres computation itself well within budget and
  points at the image transport path instead.
\end{itemize}

\textbf{This is a reasoned diagnosis, not a measurement.} The network link
has not been instrumented directly; the conclusion rests on the frame size,
the observed rate, and the published benchmark figures. It should be treated
as the leading hypothesis, not an established fact.

\subsection{The intended target: build and run on the Raspberry Pi}

Running Carolus on the Pi removes the network hop entirely --- the camera
frames never leave the machine that captures them --- which addresses both
problems above at once. The target update rate for useful autonomous
operation is \textbf{at least 10 pose updates per second}.

Two points support the move beyond the bottleneck argument:

\begin{itemize}
  \item The Pi runs Ubuntu 20.04, which is the officially supported target
  for ROS Noetic. The lab PC runs 22.04 and needs the five compatibility
  fixes documented below purely as a consequence; a Pi-side build should not
  need any of them.
  \item Planned visual-inertial work (MINS) is likewise tested only on
  Ubuntu 18/20 with Melodic/Noetic, so the Pi qualifies and the 22.04 lab PC
  does not.
\end{itemize}

\subsection{Measured on the Pi, 2026-08-04}
\label{sec:pi-benchmark}

Carolus was built and run on the Raspberry Pi 4B. The results settle the
question in favour of the Pi:

\begin{center}
\begin{tabular}{lll}
\toprule
Measurement & Lab PC & Raspberry Pi 4B \\
\midrule
Camera publish rate & 10--15\,Hz & \textbf{30.0\,Hz} \\
Carolus frames processed & $\sim$2.5/s & \textbf{$\sim$24/s} \\
CPU load while processing & --- & $\sim$61\% user, 16\% idle \\
Build & 5 compatibility fixes & \textbf{none needed} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{This is roughly ten times the lab-PC rate, and more than twice the
10\,Hz target}, which confirms the diagnosis above: the bottleneck was the
network transport, not the P4P/Ceres computation. On the Pi the frames never
leave the machine that captures them, and the camera itself publishes at its
full 30\,Hz instead of the 10--15\,Hz that survived the network hop.

The build needed \textbf{no} compatibility workarounds. The Pi runs
Ubuntu 20.04.6 with Ceres 1.14, Eigen 3.3.7 and OpenCV 4.2 already present;
\texttt{catkin\_make} completed and linked the binary directly on
\texttt{aarch64}. Available memory (7.6\,GB) was never a constraint.

\textbf{Two caveats on the figure, stated so it is not over-read.} The
$\sim$24 frames/s is a \emph{processing} rate, counted from the node's
per-frame log output with no beacon in view; a run with four LEDs visible
also executes the P4P solve per frame, which was not part of this
measurement. And the pose rate visible to a consumer is additionally gated
by the outlier filter (\texttt{filter\_size}, \texttt{fifo\_on}). The
headroom over 10\,Hz is large enough that neither is likely to be
disqualifying, but a beacon-in-view measurement of \texttt{/pose} itself
remains to be done.

\textbf{What has not yet been done:} running the full autonomous pipeline
from the Pi end to end, and deciding where the operator GUI should then
live. The procedure in the rest of this chapter still describes the lab-PC
build, which is what the rest of this project's tooling currently assumes.
For a Pi build, follow the Ubuntu 20.04 instructions in the next section and
skip the 22.04 fixes entirely, passing
\texttt{ubuntu2204\_preload:=false} at launch.

\section{Getting this project's code onto the lab PC}

Clone the project's repository onto the lab PC. Cloning into a directory
named \texttt{carolus\_ws} gives a standard catkin workspace layout, with
the ROS packages under \texttt{src/}:
...
\textbf{This is a private repository.} If you open the link above without
having been granted access, GitHub returns \textbf{404 Not Found}, not
``403 Forbidden'' --- this is deliberate on GitHub's part (it avoids
confirming that a private repository exists at all), so a 404 here means
``you do not have access yet'', not ``the link is wrong''.
...
$\to$ Add people} on the repository). You will need to give them your GitHub
username. Once added, you will receive an email invitation that must be
accepted before cloning works.
...
\section{Building on Ubuntu 20.04}

If the lab PC is on Ubuntu~20.04 (the officially supported target for ROS
Noetic), building is standard:
...
Skip the rest of this section and go to Section~\ref{sec:launching}.

\section{Building on Ubuntu 22.04 (unsupported, requires fixes)}

ROS Noetic officially targets Ubuntu~20.04 only. If the lab PC runs 22.04,
four system-level fixes are needed before \texttt{catkin\_make} succeeds, and
a fifth fix is needed at launch time.
...
On this robot, nothing changes --- the entry point is the same as before:

\begin{lstlisting}[style=shell]
roslaunch carolus_node testcarolus.launch
\end{lstlisting}

\textbf{On Ubuntu 20.04, including the Raspberry Pi}, disable the
\texttt{LD\_PRELOAD} workaround: it exists only to work around the 22.04
library mismatch described in Fix 5 above, and is unnecessary --- and
wrong-architecture --- anywhere else:
