...
\textbf{Not yet done, and deliberately not documented as if it were:}
Carolus has not yet been built or benchmarked on the Raspberry Pi on this
project. Until a Pi-side build is compiled and its actual pose rate
measured, the procedure below --- which is what has genuinely been run ---
targets the lab PC. The build steps themselves are not architecture-specific
(the same \texttt{catkin\_make} sequence applies), so the Ubuntu 20.04
instructions in the next section are the relevant ones for a Pi build; the
22.04 fixes after them are lab-PC-specific and should be skipped.
...
The current launch file, \texttt{src/carolus\_node/launch/testcarolus.launch},
reproduced below with its comments translated to English (every parameter and
value is exactly as in the repository; a disabled RViz block at the end of the
real file is omitted here):

\begin{lstlisting}[style=code,language=XML]
<launch>
    <node name="carolus_astrobee_rex" pkg="carolus_astrobee" type="carolus_astrobee" output="screen">

        <!-- Ubuntu 22.04 runtime fix -- see Fix 5 above -->
        <env name="LD_PRELOAD"
             value="/usr/lib/x86_64-linux-gnu/liblog4cxx.so.10:/usr/lib/x86_64-linux-gnu/libtinyxml2.so.6" />
        ...
        <!-- Blue LEDs: OpenCV hue (0-180) is approximately 100-140.
             Tune live against /postprocessed/image if the beacon color changes. -->
        ...
        <!-- Robot label -- a legacy name from the Astrobee simulation template
             this launch file was originally derived from. It has no functional
             effect on this robot; left as-is rather than renamed. -->
        <param name="bot_name" value="wannabee" />

        <!-- Camera intrinsics -- see Chapter on Calibration -->
        ...
        <!-- pose_topic must stay different from /pose: the C++ node advertises
             pose_topic_ as ff_msgs/VisualLandmarks and /pose as PoseStamped;
             if the two names coincide, only one publisher survives. -->
        <param name="pose_topic" value="/loc/ar/features" />

        <param name="benchtest" value="true" />

        <!-- Outlier rejection (a moving-window filter on successive poses) -->
        ...
        <!-- Known beacon geometry, metres, in Carolus's own (left-handed) frame -->
        <param name="known_points" value="0.0825 0.0 0.0 -0.0825 0.0 0.0 0.0 0.072 0.0 0.0 0.0 0.0555" />

    </node>

    <node pkg="tf2_ros" type="static_transform_publisher" name="static_honey_body_tf"
