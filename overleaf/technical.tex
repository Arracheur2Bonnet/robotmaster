...
\begin{itemize}
  ...
  \item \textbf{Lab PC:} runs the operator GUI, the Carolus pose-estimation
  node, and (planned) the localization back end. Not physically connected to
  the robot; communicates with the Pi over the local network.
...
Carolus (the ROS package name is \texttt{carolus\_astrobee}) is built on the
\textbf{lab PC}, not the Pi. It has no SDK connection of its own --- it only
consumes \texttt{/camera/color/image\_raw} (published by
\texttt{rm\_cam\_beacon.py} over the network from the Pi, Chapter~\ref{ch:camera})
and publishes a pose.

\section{Getting this project's code onto the lab PC}

Clone the project's repository onto the lab PC --- it contains
\texttt{carolus\_node/} (the launch files and configuration),
\texttt{libuvgs\_astrobee/} (Carolus's own C++ source, already patched for
this robot), \texttt{ff\_msgs/} (a message-definition dependency), and
...
This is a private repository; you need collaborator access (ask the project
supervisor) and an SSH key or GitHub credentials of your own --- never a
token pasted directly into a clone command shared between people.
...
The full, current launch file, \texttt{carolus\_node/launch/testcarolus.launch}:
...
\texttt{lb\_hue}, \texttt{ub\_hue} & Hue range (degrees, 0--360) the beacon's LEDs must fall within. \\
\bottomrule
\end{tabular}
\end{center}
