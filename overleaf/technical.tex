...
\begin{itemize}
  ...
  camera/beacon-tracking node, \textbf{and runs Carolus itself} --- so the
  full perception pipeline is onboard and the robot needs no network link to
  estimate its pose.
  ...
  communicates with the Pi over the local network. \textbf{It is not needed
  for Carolus itself} --- pose estimation runs on the Pi
  (Section~\ref{sec:where-carolus-runs}), so the robot does not depend on a
  network link to the PC to localise.
...
\runson{the \textbf{Raspberry Pi 4B} on the robot. A lab-PC build is also
documented, as a fallback for development --- see
Section~\ref{sec:where-carolus-runs}.}

Carolus (the ROS package name is \texttt{carolus\_astrobee}) is built and run
\textbf{on the Raspberry Pi}, alongside the camera bridge. It has no SDK
...
publishes a pose on \texttt{/pose}.

\section{Where Carolus runs}
\label{sec:where-carolus-runs}

\textbf{Run Carolus on the Pi.} Both machines work, but the Pi is the correct
target and the difference is not marginal.

\subsection{Why the Pi}

\begin{itemize}
  \item \textbf{The robot stays autonomous.} If Carolus runs on the lab PC,
  every pose estimate depends on a live network link, so the robot can never
  leave the PC's network reach. On the Pi it does not need the PC at all.
  \item \textbf{It is roughly ten times faster.} Running on the PC means every
  frame --- \emph{uncompressed} \texttt{sensor\_msgs/Image} at
  $1280\times720$, about 2.76\,MB --- crosses the network before Carolus can
  touch it. That transport, not the P4P/Ceres computation, was the
  bottleneck. On the Pi the frames never leave the machine that captured
  them.
  \item \textbf{The build is simpler.} The Pi runs Ubuntu 20.04, ROS Noetic's
  officially supported target. None of the five compatibility fixes the
  22.04 lab PC needs (Section~\ref{sec:build2204}) apply.
\end{itemize}

\subsection{Measured, 2026-08-04}
\label{sec:pi-benchmark}

\begin{center}
\begin{tabular}{lll}
\toprule
Measurement & Lab PC & \textbf{Raspberry Pi 4B} \\
\midrule
...
Compatibility fixes needed & 5 & \textbf{none} \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Two caveats, so the figure is not over-read.} The $\sim$24 frames/s
is a \emph{processing} rate, counted from the node's per-frame log output
with no beacon in view; a run with four LEDs visible also executes the P4P
solve per frame. And the pose rate a consumer sees is further gated by the
outlier filter (\texttt{filter\_size}, \texttt{fifo\_on}). A beacon-in-view
measurement of \texttt{/pose} itself remains to be done.

\subsection{When to build on the lab PC instead}

The lab-PC path is kept in this manual as a development fallback --- editing
and rebuilding C++ is faster on a workstation than over SSH on a Pi, and the
GUI tooling currently runs there. It is documented in
Section~\ref{sec:build2204}. For anything resembling normal operation,
build on the Pi.
...
and what gives the throughput measured in
